...
class Tournament:
    def __init__(self):
        ...
        self.pairings = []
        self.results = []
        self.pairing_possible = True  # NEW: track if pairing is possible
    ...
    # ---------- BACKTRACKING ENGINE (FALLBACK) ----------
    def _find_pairing_backtracking(self, players):
        """Recursive backtracking for fallback when greedy fails."""
        if not players:
            return []
        if len(players) % 2 == 1:
            return None
        ...
        for i in range(1, len(players)):
            ...
            result = self._find_pairing_backtracking(remaining)
            if result is not None:
                return [(first, opponent)] + result
        return None

    def _try_global_pairing(self):
        """Try to pair all players with backtracking, handling odd count with bye."""
        total = len(self.players)
        if total % 2 == 0:
            result = self._find_pairing_backtracking(self.players[:])
            if result is not None:
                return result
            return None
        else:
            # Try each eligible player for bye
            eligible = [p for p in self.players if not p.has_bye]
            if not eligible:
                eligible = self.players[:]
            eligible.sort(key=lambda p: p.rating)
            for bye_candidate in eligible:
                rest = [p for p in self.players if p != bye_candidate]
                result = self._find_pairing_backtracking(rest)
                if result is not None:
                    bye_candidate.points += 1.0
                    bye_candidate.has_bye = True
                    return result + [(bye_candidate, None)]
            return None

    # ---------- ROUND 2, 3... (ORIGINAL GREEDY + FALLBACK) ----------
    def pair_next_round(self):
        if len(self.players) < 2:
            return []

        # ----- ORIGINAL GREEDY LOGIC (PRESERVED) -----
        groups = {}
        ...
        # ----- VALIDATION & FALLBACK (NEW) -----
        # Check if all players are paired (or exactly one bye)
        paired_names = set()
        for w, b in all_pairs:
            paired_names.add(w.name)
            if b:
                paired_names.add(b.name)
        unpaired_players = [p for p in self.players if p.name not in paired_names]

        if unpaired_players:
            # Greedy failed! Use backtracking fallback.
            fallback_result = self._try_global_pairing()
            if fallback_result is not None:
                self.pairings = fallback_result
                self.results = [None] * len(fallback_result)
                return fallback_result
            else:
                # Even backtracking failed -> Pairing Ended.
                self.pairings = []
                self.results = []
    ...
    # ---------- MAX ROUNDS ----------
    def get_max_possible_rounds(self):
        n = len(self.players)
        if n < 2:
            return 0
        # Theoretical max: n-1 for even, n for odd (but Swiss usually n-1)
        # We'll return a reasonable estimate.
        return n - 1
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Chess Swiss - Original + Pairing End Detection")
    ...
    def build_ui(self):
        ...
        ttk.Button(top_frame, text="Max Rounds", command=self.show_max_rounds).grid(row=0, column=7, padx=5)
    ...
    def record_result(self, result_code):
        if not self.tournament.pairing_possible:
            messagebox.showinfo("Info", "Tournament ended. No more results.")
            return
    ...
    def round1(self):
        ...
        self.tournament.pairing_possible = True

    def next_round(self):
        if not self.tournament.pairing_possible:
            messagebox.showinfo("Info", "Pairing ended. Cannot generate more rounds.")
            return
        ...
        if pairs is None:
            self.tournament.pairing_possible = False
    ...
    def show_max_rounds(self):
        max_rounds = self.tournament.get_max_possible_rounds()
        messagebox.showinfo("Maximum Rounds",
            f"Maximum possible rounds: {max_rounds}\n\n"
            f"(With {len(self.tournament.players)} players)")
